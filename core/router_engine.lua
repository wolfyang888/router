...
function RouterEngine:register_interface(name, interface, adapter)
    ...
    -- 将 adapter 设置到 interface 对象上
    if interface then
        interface.adapter = adapter
    end
    log.info(string.format("Interface registered: %s", name))
end
...
function RouterEngine:probe_route(interface_name, test_message)
    ...
    local status = interface:get_status()
    if status.status ~= "connected" then
        log.error(Error.get_message(Error.INTERFACE_NOT_CONNECTED), "interface: %s", interface_name)
    ...
    if result then
        ...
        log.debug(string.format("Route probe successful: %s (latency: %.1fms)", interface_name, latency))
        return true, latency
    else
        metrics:record_failure()
        log.error(Error.get_message(Error.INTERFACE_SEND_FAILED), "interface: %s", interface_name)
        return false, 0
    end
end
...
function RouterEngine:add_routing_rule(rule)
    ...
    log.info(string.format("Routing rule added: %s (priority: %d)",
...
function RouterEngine:set_routing_strategy(strategy_name)
    if self.routing_strategies[strategy_name] then
        ...
        log.info(string.format("Routing strategy set to: %s", strategy_name))
        return true
    else
        log.error(Error.get_message(Error.INVALID_ROUTING_STRATEGY), "strategy: %s", strategy_name)
        return false
    end
end
...
function RouterEngine:add_custom_strategy(name, strategy_func)
    if type(strategy_func) == "function" then
        ...
        log.info(string.format("Custom routing strategy added: %s", name))
        return true
    else
        log.error(Error.get_message(Error.CUSTOM_STRATEGY_NOT_FUNCTION))
        return false
    end
end
...
function RouterEngine:send_with_routing(message, device_id,
    ...
    if not interface_name then
        Error.log(Error.NO_AVAILABLE_ROUTE, "device: %s", device_id)
        return false
    end
    ...
    if result then
        ...
        Error.debug(string.format(
            "Message sent via %s to device %s", interface_name, device_id))
        return true
    else
        metrics:record_failure()
        Error.log(Error.INTERFACE_SEND_FAILED, "interface: %s, device: %s", interface_name, device_id)
...
function RouterEngine:parse_device_address(addr_str)
    if not addr_str then
        Error.log(Error.INVALID_PARAM, "地址字符串为空")
        return nil
    end
...
function RouterEngine:forward_to_device(message, src_device, dst_device)
    if not self.topology then
        Error.log(Error.ROUTE_NOT_FOUND, "拓扑未初始化")
        return false
    end

    local path = self:discover_device_path(src_device, dst_device)
    if not path then
        Error.log(Error.NO_PATH_FOUND, "%s -> %s", src_device, dst_device)
        return false
    end

    local materialized = self:materialize_path(path)
    if #materialized == 0 then
        Error.log(Error.NO_PATH_FOUND, "无可用的接口路径")
        return false
    end
...
function RouterEngine:register_device(device_id, interfaces)
    ...
    for _, iface in ipairs(interfaces or {}) do
        local iface_type, iface_port
        if type(iface) == "string" then
            iface_type = iface
            iface_port = ""
        else
            iface_type = iface.type or ""
            iface_port = iface.port or ""
        end
        local addr = device_id .. ":" .. iface_type .. ":" .. iface_port
        self.device_interfaces[device_id][iface_type] = {
            address = addr,
            config = iface,
            priority = (type(iface) == "table" and iface.priority) or 50
        }
    end
...
function RouterEngine:unregister_device(device_id)
    if self.devices[device_id] then
        ...
        log.info("Device unregistered: " .. device_id)
        return true
    end
    return false
end
